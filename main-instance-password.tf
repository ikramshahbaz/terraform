...
resource "azurerm_managed_disk" "manageddisk" {
  name                 = "${azurerm_virtual_machine.main2.name}-disk1"
...
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec" {
command = "ansible-playbook -u testadmin -i '${data.azurerm_public_ip.vm.ip_address}', --extra-vars 'ansible_password=Password1234' provision.yml"
  }
}
